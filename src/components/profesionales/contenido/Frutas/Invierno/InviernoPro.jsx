...
import './assets/css/invierno.css'
import { menuFrutas } from './assets/data/dataFrutas'
import prevBoton from '../../../contenido/assets/images/f-izq.png'
import textInvierno from './assets/images/Invierno.png'
import instrucciones from './assets/images/Instruccion.png'
...
function InviernoPro() {
  const [buttons, setButtons] = useState(menuFrutas)
  const [str_id_act, setStrIdAct] = useState('frut_invierno')
  ...
  const traerDatos = () => {
    ...
    actService.existeActivity(data).then(response => {
      if (isMounted) {
        ...
        const amber = contestarExiste[0]?.vc_estatus;
        setTxtStatus(amber)
  ...
  const handleButtonClick = (id) => {
    ...
    const ones = str_data.filter(pos => pos === "1");
    if (ones.length > 3) {
      avanceF = 'complete'
    } else {
      avanceF= 'incomplete'
  ...
  return (
    <HorizontalPage>
      <div className='invierno__contenido-menu'>
        {
          loading ? (
            <h1>Cargando..</h1>
          )
            :
            (
              <div className='invierno__grid-template'>
                <div className='invierno__cocina'>
                  <div className='invierno__intruccion'>
                    <img src={instrucciones} loading="lazy" />
                  </div>
                  <div className='invierno__menu-cocina'>
                    <div className='invierno__cinco'>
                      {buttons.map(({ id, imagen, to, statusF }) => (
                        <div className='frutas__image' key={id}>
                          <Link
                            className='frutas__link'
                            to={to}
                            onClick={() => { handleButtonClick(id) }}>
                            <img
                              className={statusF === 'disabled' ? 'boxShadowAnimate invierno__frutitas' : 'boxShadowAnimate-enabled invierno__frutitas'}
                              src={imagen}
                              loading="lazy" />
                          </Link>
                        </div>
                  ...
                  <div>
                    <img src={textInvierno} className='invierno__texto-find' loading="lazy" />
                  </div>
                </div>
                <div className='video__contenido-principal'>
                  <div className='video__boton-siguiente'>
                    <Link to='/profesionales/frutas/estacion'><img src={prevBoton} loading="lazy" /></Link>
                  </div>
                  <div className='video__boton-siguiente'>
                  </div>
                </div>
