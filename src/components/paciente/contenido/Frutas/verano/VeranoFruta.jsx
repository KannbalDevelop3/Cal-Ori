...
import { menuFrutasVerano } from './assets/data/OptFrutasVerano'
...
import titleEstacion from './assets/images/VERANO.png'
import './assets/styles/veranofrutas.css'

const VeranoFruta = () => {
    const ruta = '/paciente/frutas/estacion';
    const [buttons, setButtons] = useState(menuFrutasVerano)
    const [str_id_act, setStrIdAct] = useState('frut_verano')
    ...
    return (
        <MenuVerduraOFruta
            ...
            width1='85%' width2='50%' className='estacion__texto-verano' rutaPrev={ruta} left=''>
            {
                loading ? (<h1>Cargando..</h1>)
                    :
                    (
                        <div className='verano-frutass__menu-estacion'>
                            {buttons.map(({ id, imagen, disabled, to, statusF }) => (
                                <div className='verano-frutass__center-div' key={id}>
                                    <button className={`verano-frutass__boton-opt-e`}>
                                        <Link
                                            to={`${disabled === false ? `/paciente/frutas/estacion/primavera/${to}` : ''}`}
