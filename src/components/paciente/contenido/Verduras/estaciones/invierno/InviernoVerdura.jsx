...
import './assets/styles/menuVerdura.css'
import fondoBack from './assets/images/menu/FONDO.png'
...
const InviernoVerdura = () => {
    const ruta = '/paciente/verduras/estacion';
    const [buttons, setButtons] = useState(menuVerduras)
    const [str_id_act, setStrIdAct] = useState('verd_invier')
    ...
    const enviarDatos = async () => {
        ...
        await actService.crearActivity(data).then(response => {
            console.log(response.data[0]?.exito);
    ...
    const traerDatos = () => {
        ...
        actService.existeActivity(data).then(response => {
            if (isMounted) {
                ...
                const estadoA = contestarExiste[0]?.vc_estatus;
                setTxtStatus(estadoA)
    ...
    return (
        <MenuVerduraOFruta
            ...
            width1='80%' width2='50%' className='' rutaPrev={ruta}>
            <div className='menu-verduras__menu-estacion'>
                {buttons.map(({ id, imagen, disabled, to, statusFruit }) => (
                    <div className='menu-verduras__center-div' key={id}>
                        <button className={`menu-verduras__boton-opt-e`}>
                            <Link
                                to={`${disabled === false ? `/paciente/verduras/estacion/invierno/${to}` : ''}`}
                                onClick={() => { handleButtonClick(id) }}
                            >
                                <img
                                    style={{
                                        cursor: disabled === true ? 'no-drop' : 'pointer',
...
export default InviernoVerdura